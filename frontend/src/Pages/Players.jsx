...
import {
  ...
  ChevronLeft, ChevronRight, Users, TrendingUp, AlertTriangle, UserX, Download, UserPlus,
} from 'lucide-react';
...
const STATUS = {
  ...
  UNREACHABLE:     { bg: '#fdf4ff', text: '#86198f', border: '#e879f9', dot: '#a21caf', label: 'Unreachable' },
...
const TH = {
  padding: '10px 16px', fontWeight: '700', fontSize: '11px', color: C.grayLt,
          ...
          {TABS.map(tab => {
            ...
            return (
              <button key={tab.id} onClick={() => { setFilterTab(tab.id); setCurrentPage(1); }}
                style={{ padding: '14px 16px', background: 'none', border: 'none', fontWeight: active ? '700' : '600', fontSize: '13px', color: active ? C.sky : C.gray, borderBottom: `2px solid ${active ? C.sky : 'transparent'}`, marginBottom: '-1px', cursor: 'pointer', transition: 'all .15s', whiteSpace: 'nowrap', fontFamily: 'inherit' }}>
                {tab.label}
              </button>
            );
          })}
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={9} style={{ padding: '60px', textAlign: 'center', color: C.grayLt }}>
                ...
                <tr>
                  <td colSpan={9} style={{ padding: '60px', textAlign: 'center', color: C.grayLt, fontSize: '14px' }}>
                    No players found
                  </td>
                </tr>
                ...
                return (
                  <tr key={player.id}
                    ...
                    <td style={{ padding: '13px 16px', borderBottom: `1px solid ${C.bg}` }}>
                      {player.addedBy ? (
                        <AddedByBadge addedBy={player.addedBy} />
                      ) : (
                        <span style={{ fontSize: '11px', color: C.grayLt, fontStyle: 'italic' }}>—</span>
                      )}
                    </td>
