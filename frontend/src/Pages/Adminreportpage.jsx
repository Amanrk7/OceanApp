...
function AuditVerification({ endSnapshot, transactions }) {
    ...
    const ptDiscrepancy = endSnapshot.gameDiscrepancy ?? 0;

    const cashOk = Math.abs(cashDiscrepancy) < 0.02;
    const ptsOk = Math.abs(ptDiscrepancy) < 1;
...
// ══════════════════════════════════════════════════════════════
// PDF PRINT (same logic as original, updated styling)
// ══════════════════════════════════════════════════════════════
function printReport(report, date) {
    ...
    const taskRows = (dayTasks || []).map(t => {
      ...
      <td>${pct !== null ? `${t.currentValue} / ${t.targetValue} (${Math.min(100, pct)}%)` : "—"}</td>
    </tr>`;
    }).join("");
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  th{background:#f8fafc;text-align:left;padding:8px 10px;font-weight:700;color:#64748b;font-size:10px;text-transform:uppercase;border-bottom:2px solid #e2e8f0}
