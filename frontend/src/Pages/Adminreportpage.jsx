...
function DisplayTypeBadge({ type }) {
    const map = {
        ...
        "Match Bonus": { bg: "#fffbeb", text: "#92400e" },
        "Special Bonus": { bg: "#fdf4ff", text: "#7e22ce" },
        "Streak Bonus": { bg: "#eff6ff", text: "#1d4ed8" },
        "Referral Bonus": { bg: "#f0fdf4", text: "#166534" },
        "Bonus": { bg: "#fffbeb", text: "#92400e" },
        // fallback for raw DB values
        "DEPOSIT": { bg: "#dcfce7", text: "#166534" },
        "WITHDRAWAL": { bg: "#fee2e2", text: "#991b1b" },
        "BONUS": { bg: "#fffbeb", text: "#92400e" },
    };
    const s = map[type] || { bg: "#f1f5f9", text: "#475569" };
    return (
        <span style={{ padding: "2px 8px", borderRadius: "5px", fontSize: "10px", fontWeight: "700", background: s.bg, color: s.text, whiteSpace: "nowrap" }}>
            {type}
        </span>
    );
}
                ...
                <div style={{ overflowX: "auto" }}>
                    {!shift.transactions?.length
                        ...
                        : (
                            <table style={{ width: "100%", borderCollapse: "collapse" }}>
                                <thead>
                                    {/* ← ADD "Bonus Type", "Game Pts", change "Wallet" header */}
                                    <tr>{["Time", "Player", "Type", "Bonus Type", "Game", "Game Pts", "Wallet", "Amount", "Fee", "Balance After"].map(h => <th key={h} style={{ ...TH, padding: "8px 12px", fontSize: "10px" }}>{h}</th>)}</tr>
                                </thead>
                                <tbody>
                                    {shift.transactions.map(t => (
                                        <tr key={t.id} onMouseEnter={e => e.currentTarget.style.background = "#fafbfc"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                                            ...
                                            {/* ← USE displayType instead of t.type */}
                                            <td style={{ ...TD, padding: "9px 12px" }}>
                                                <DisplayTypeBadge type={t.displayType || t.type} />
                                            </td>

                                            {/* ← NEW: bonus subtype */}
                                            <td style={{ ...TD, padding: "9px 12px", fontSize: "11px", color: "#7c3aed" }}>
                                                {t.bonusType
                                                    ? <span style={{ padding: "2px 7px", borderRadius: "4px", fontSize: "10px", fontWeight: "700", background: "#f5f3ff", color: "#7c3aed" }}>{t.bonusType}</span>
                                                    : <span style={{ color: "#cbd5e1" }}>—</span>}
                                            </td>

                                            {/* ← USE gameName (parsed from notes) */}
                                            <td style={{ ...TD, padding: "9px 12px", fontSize: "11px", color: "#94a3b8" }}>{t.gameName || t.game?.name || "—"}</td>

                                            {/* ← NEW: game stock before → after */}
                                            <td style={{ ...TD, padding: "9px 12px", fontSize: "11px" }}>
                                                {t.gameStockBefore != null
                                                    ? <span style={{ color: "#64748b" }}>{t.gameStockBefore.toFixed(0)} → <b style={{ color: t.gameStockAfter < t.gameStockBefore ? "#dc2626" : "#16a34a" }}>{t.gameStockAfter?.toFixed(0)}</b></span>
                                                    : <span style={{ color: "#cbd5e1" }}>—</span>}
                                            </td>

                                            {/* ← USE walletMethod + walletName instead of t.paymentMethod */}
                                            <td style={{ ...TD, padding: "9px 12px", fontSize: "11px", color: "#64748b" }}>
                                                {t.walletName
                                                    ? <span>{t.walletMethod}<br /><span style={{ color: "#94a3b8" }}>{t.walletName}</span></span>
                                                    : <span style={{ color: "#cbd5e1" }}>—</span>}
                                            </td>
                                            ...
                                            {/* ← USE t.fee (now parsed from notes by enrichShift) */}
                                            <td style={{ ...TD, padding: "9px 12px", fontSize: "11px" }}>
                                                {t.fee > 0
                                                    ? <span style={{ color: "#f59e0b", fontWeight: "700" }}>−{fmtMoney(t.fee)}</span>
                                                    : <span style={{ color: "#cbd5e1" }}>—</span>}
                                            </td>
