...
import {
    ...
    ArrowDownLeft, ArrowUpRight, Zap, Gift, Star, Users,
} from "lucide-react";
import { api } from "../api";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.transaction?.referralBonus) {
                ...
                msg += ` Referral bonus of ${fmt(rb.amount)} also sent to ${rb.referrerName}.`;
            }
            setSuccess(msg);
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "28px", maxWidth: "inherit" }}>
            ...
            <div style={CARD}>
                ...
                <div style={{ display: "flex", gap: "10px", marginBottom: "22px" }}>
                    {[
                        ...
                        <button key={id} type="button"
                            onClick={() => { set("txType", id); setError(""); setSuccess(""); setUndoSuccess(""); }}
                            style={{
                ...
                <div style={{
                    ...
                    <div>
                        <p style={{ fontWeight: "700", color: isDeposit ? "#14532d" : "#7f1d1d", margin: "0 0 2px", fontSize: "14px" }}>
                ...
                <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                        ...
                        <div ref={dropRef} style={{ position: "relative" }}>
                            ...
                            {showDrop && !searching && (
                                <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, zIndex: 60, background: "#fff", border: "1px solid #e2e8f0", borderRadius: "10px", boxShadow: "0 8px 24px rgba(15,23,42,.12)", overflow: "hidden", maxHeight: "260px", overflowY: "auto" }}>
                                    {results.length === 0
                                        ? <div style={{ padding: "14px 16px", color: "#94a3b8", fontSize: "13px" }}>No players found</div>
                                        : results.map(p => (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                        <div>
                            ...
                            <input type="number" placeholder="0.00" min="0.01" step="0.01"
                                ...
                                style={{ ...INPUT, borderColor: cashoutOverLimit ? "#fca5a5" : "#e2e8f0" }} />
                            {cashoutOverLimit && (
                    ...
                    {isDeposit && (
                        <div>
                            <label style={LABEL}>Game {anyBonus ? "* (required for bonus deduction)" : "(optional)"}</label>
                            <div style={{ position: "relative" }}>
                                <select value={form.gameId} onChange={e => set("gameId", e.target.value)}
                                    ...
                                    {games.map(g => (
                                        <option key={g.id} value={g.id} disabled={g.pointStock <= 0}>
                                            {g.name}  ({(g.pointStock ?? 0).toFixed(0)} pts){g.pointStock <= 0 ? " — EMPTY" : ""}
                                        </option>
                                    ))}
                                </select>
                                <ChevronDown style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", width: "14px", height: "14px", color: "#94a3b8", pointerEvents: "none" }} />
                    ...
                    <div>
                        ...
                        <textarea placeholder="Add any notes…" rows={2}
                            value={form.notes}
                    ...
                    <div style={{ display: "flex", gap: "12px", paddingTop: "4px" }}>
                        <button type="button"
                            onClick={() => { setForm(EMPTY); setQuery(""); clearPlayer(); setError(""); setSuccess(""); setUndoSuccess(""); }}
            ...
            <div style={{ ...CARD, padding: 0, overflow: "hidden" }}>
                ...
                {ledgerLoading ? (
                    ...
                    <div style={{ padding: "40px", textAlign: "center", color: "#94a3b8", fontSize: "13px" }}>No transactions yet</div>
                ) : (
                    <div style={{ overflowX: "auto", maxHeight: "520px", overflowY: "scroll", scrollbarWidth: "thin" }}>
                                ...
                                {ledger.map((tx, i) => {
                                    ...
                                    return (
                                        <tr key={tx.id ?? i}
                                            ...
                                            onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                                            style={{ borderBottom: "1px solid #f1f5f9", opacity: tx.status === "CANCELLED" ? 0.6 : 1 }}>

                                            <td style={{ padding: "11px 14px", color: "#0ea5e9", fontWeight: "600", fontSize: "12px" }}>{tx.id}</td>
                                            ...
                                            <td style={{ padding: "11px 14px" }}>
                                                <span style={{ display: "inline-block", padding: "3px 9px", background: typeColor.bg, color: typeColor.text, borderRadius: "6px", fontSize: "12px", fontWeight: "600", whiteSpace: "nowrap" }}>
                                                    {displayType}
                                                </span>
                                            </td>
                                            ...
                                            <td style={{ padding: "11px 14px", minWidth: "150px" }}>
                                                {(tx.walletMethod || tx.walletName) ? (
                                                    <div>
                                                        <div style={{ fontSize: "12px", fontWeight: "600", color: "#0f172a" }}>{tx.walletMethod || "Unknown"}</div>
                                                        <div style={{ fontSize: "11px", color: "#94a3b8", marginTop: "1px" }}>{tx.walletName || "—"}</div>
                                                    </div>
                                                ) : (
                                                    <span style={{ color: "#cbd5e1", fontSize: "12px" }}>No wallet</span>
                                                )}
                                            </td>

                                            <td style={{ padding: "11px 14px", fontSize: "12px", whiteSpace: "nowrap", minWidth: "140px" }}>
                                            ...
                                            <td style={{ padding: "11px 14px" }}>
                                                <span style={{ display: "inline-block", padding: "3px 9px", borderRadius: "6px", fontSize: "12px", fontWeight: "600", background: tx.status === "COMPLETED" ? "#dcfce7" : tx.status === "CANCELLED" ? "#fee2e2" : "#fef3c7", color: tx.status === "COMPLETED" ? "#166534" : tx.status === "CANCELLED" ? "#991b1b" : "#92400e" }}>
                                                    {tx.status}
                                                </span>
                                            </td>
                                            ...
                                            <td style={{ padding: "11px 14px" }}>
                                                ...
                                                {isUndoing && <span style={{ fontSize: "12px", color: "#94a3b8" }}>Reversing…</span>}
                                            </td>
                                        </tr>
                                    );
                                })}
