 ...
 *   1. All Unicode chars unsupported by jsPDF Helvetica replaced with ASCII:
 *        · (U+00B7) → |      → (U+2192) → ->     ✓ (U+2713) → OK
 *        ⚠ (U+26A0) → (!)   − (U+2212) → -      — (U+2014) → -
 *        – (U+2013) → -      ≥ (U+2265) → >=     ★ (U+2605) → *
 *        ☆ (U+2606) → .
 *   2. getRoleLabel() handles both old TEAM1-8 and new SLOT_N format
 *   3. kpiGrid() skips empty placeholder cards (no more ghost boxes)
 *   4. KPI padding while-loop removed from printShiftPDF
 *   5. Effort bar in feedback page uses ASCII fill char
 *   6. Star rating replaced with numeric display
 *   7. Header sub-text separators changed to  |  throughout
 * ─────────────────────────────────────────────────────────────────────────────
 */
...
// ═══════════════════════════════════════════════════════════════════════════════
// FORMATTERS  (all return ASCII-safe strings)
// ═══════════════════════════════════════════════════════════════════════════════
const fmt$ = (n) =>
...
// ─── Role label — handles both old TEAM1-8 and new SLOT_N ───────────────────
...
function getRoleLabel(role) {
  if (!role) return '--';
  if (ROLE_LABEL_MAP[role]) return ROLE_LABEL_MAP[role];
  const m = String(role).match(/^SLOT_(\d+)$/);
...
/**
 * KPI card grid.
 * Empty items (label === '' AND value === '') are silently skipped — no ghost boxes.
 */
function kpiGrid(doc, items, y, cols = 5) {
  // Filter out empty placeholder items to prevent ghost boxes
  ...
  validItems.forEach(({ label, value, color = C.blue, sub }, i) => {
    const col = i % cols;
    ...
    doc.text(fitText(doc, value, colW - 8), textCenter(x), ky + 10, { align: 'center' });

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6);
    doc.setTextColor(...C.gray);
    doc.text(fitText(doc, (label || '').toUpperCase(), colW - 6), textCenter(x), ky + 15.5, { align: 'center' });

    if (sub) {
      doc.setFontSize(5.5);
      doc.setTextColor(...C.muted);
      doc.text(fitText(doc, sub, colW - 6), textCenter(x), ky + 19.5, { align: 'center' });
    }
  });
...
/**
 * Status banner — uses ASCII OK / (!) instead of Unicode check/warning glyphs.
 */
function statusBanner(doc, y, isOk, label, sub) {
  ...
  // Small filled indicator circle
  doc.setFillColor(...accent);
  doc.circle(22.5, y + 7, 2.8, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7);
  doc.setTextColor(...C.white);
  doc.text(isOk ? 'OK' : '!', 22.5, y + 7.7, { align: 'center' });

  // Label
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(9);
  doc.setTextColor(...accent);
  doc.text(fitText(doc, label, W - 55), 28, y + 5.5);

  if (sub) {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.setTextColor(...(isOk ? C.greenDk : C.redDk));
    doc.text(fitText(doc, sub, W - 46), 28, y + 10.5);
  }
  return y + h + 4;
}
...
function addFooters(doc, reportType, rangeLabel) {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    doc.setTextColor(...C.muted);
    doc.text(
      'CONFIDENTIAL  |  ' + reportType + '  |  ' + rangeLabel + '  |  Generated ' + genStamp(),
      14, H - 4,
    );
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7);
...
export async function downloadPlayersPDF(filterTab, searchTerm) {
  ...
  drawHeader(doc, {
    ...
    sub1: 'Generated: ' + genStamp() + '  |  ' + players.length + ' player' + (players.length !== 1 ? 's' : '') + ' exported',
    sub2: [
  ...
  autoTable(doc, {
    ...
    body: players.map((p, i) => {
      const soc = p.socials || {};
      const present = SOCIAL_KEYS
        .filter(k => soc[k])
        .map(k => k === 'x' ? 'X/Twitter' : k.charAt(0).toUpperCase() + k.slice(1))
        .join(', ') || '--';
      return [
        i + 1,
        p.name || '--',
...
export function printDailyReport(report, date) {
  ...
  const playersAdded = (report.teams || []).reduce(
    (a, t) => a + (t.shifts || []).reduce((b, sh) => b + (sh.stats?.playersAdded || 0), 0), 0,
  );
  ...
  drawHeader(doc, {
    ...
    sub2: (s.totalShifts ?? 0) + ' shift' + ((s.totalShifts ?? 0) !== 1 ? 's' : '') +
          '  |  ' + (s.transactionCount ?? 0) + ' transactions  |  Generated ' + genStamp(),
  });
  ...
  if (dayTxns.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: dayTxns.map(t => [
        fmtTime(t.createdAt),
        t._memberName + ' (' + getRoleLabel(t._teamRole) + ')',
  ...
  if (dayExp.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: dayExp.map(e => [
        fmtTime(e.createdAt),
        e._memberName + ' (' + getRoleLabel(e._teamRole) + ')',
        e.details || '--',
  ...
  if (dayTake.length > 0) {
    ...
    const byPerson = dayTake.reduce((a, t) => {
      const p = t.takenBy || '--';
      if (!a[p]) a[p] = { total: 0, count: 0 };
      a[p].total += parseFloat(t.amount ?? 0);
      a[p].count++;
      return a;
    }, {});
  ...
  const shiftsWithFeedback = (report.teams || []).flatMap(team =>
    (team.shifts || [])
      ...
      .map(sh => ({
        ...sh,
        _teamRole: team.role,
        _memberName: sh.displayMember?.name || team.member?.name || '--',
      }))
  );
  ...
  if (shiftsWithFeedback.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: shiftsWithFeedback.map(sh => {
        ...
        return [
          sh._memberName,
          getRoleLabel(sh._teamRole),
          fmtTime(sh.startTime) + ' - ' + (sh.isActive ? 'Active' : fmtTime(sh.endTime)),
          ci.effortRating != null ? ci.effortRating + ' / 10' : '--',
          parsedNotes.effortReason || (ci.additionalNotes?.startsWith('{') ? '--' : (ci.additionalNotes || '--')),
...
export function printRangeReport(reports, startDate, endDate) {
  ...
  if (allTakeouts.length > 0) {
    ...
    const byPerson  = allTakeouts.reduce((a, t) => {
      const p = t.takenBy || '--';
      if (!a[p]) a[p] = { total: 0, count: 0 };
      a[p].total += parseFloat(t.amount ?? 0);
      a[p].count++;
    ...
    const detailY = sectionHead(
      doc,
      'All Takeout Records -- ' + allTakeouts.length + ' Entries',
      (doc.lastAutoTable?.finalY ?? recY + 30) + 8,
      C.redDk,
    );
    autoTable(doc, {
      ...tbl(detailY),
  ...
  reports.forEach((r, rIdx) => {
    const allShiftsR = (r.teams || []).flatMap(team =>
      (team.shifts || []).map(sh => ({
        ...sh,
        _teamRole: team.role,
        _memberName: sh.displayMember?.name || team.member?.name || '--',
...
export async function printShiftPDF(shift) {
  ...
  let endSnap   = {};
  let feedback  = {
    effortReason: '', improvements: '', workSummary: '',
  ...
  const roleLabel  = getRoleLabel(shift.teamRole);
  const dateLabel  = fmtDate(shift.startTime);
  const dateShort  = fmtDateShort(shift.startTime);
  // NOTE: use 'to' instead of '->' to avoid any font rendering surprise
  const timeRange  = fmtTime(shift.startTime) + ' to ' + fmtTime(shift.endTime) +
  ...
  // Build KPI items — NO padding, kpiGrid handles uneven counts cleanly
  const kpiItems = [
  ...
  if (totalFees > 0)      kpiItems.push({ label: 'Total Fees',      value: fmt$(totalFees),      color: C.amber });
  if (totalExpAmt > 0)    kpiItems.push({ label: 'Expenses',        value: fmt$(totalExpAmt),    color: C.amber,  sub: expenses.length + ' items' });
  if (totalTakeoutAmt > 0) kpiItems.push({ label: 'Profit Takeouts', value: fmt$(totalTakeoutAmt), color: C.redDk, sub: takeouts.length + ' records' });
  if (ptsReloaded > 0)    kpiItems.push({ label: 'Pts Reloaded',   value: '+' + ptsReloaded + ' pts', color: C.violet });
  kpiItems.push({ label: 'Transactions', value: String(transactions.length || stx.transactionCount || 0), color: C.blue });
  if (effort != null)     kpiItems.push({ label: 'Effort Rating',   value: effort + ' / 10', color: effort >= 8 ? C.green : effort >= 5 ? C.amber : C.red });

  const kpiY = kpiGrid(doc, kpiItems, 46, 5);

  // Manager rating (numeric, no star glyphs)
  let afterRatingY = kpiY;
  if (shift.rating) {
    const overallRating = shift.rating.overallRating ?? 0;
    const ratingColor   = overallRating >= 4 ? C.green : overallRating >= 3 ? C.amber : C.red;
    let ry = sectionHead(doc, 'Manager Rating', kpiY + 2, C.violet);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(14);
    doc.setTextColor(...ratingColor);
    doc.text(overallRating.toFixed(1) + ' / 5.0', 21, ry + 5);
    doc.setFontSize(9);
    doc.setTextColor(...C.slate);
    doc.text('(' + Math.round(overallRating) + ' out of 5)', 52, ry + 5);
    if (shift.rating.feedback) {
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8.5);
      doc.setTextColor(...C.slate);
      doc.text(fitText(doc, shift.rating.feedback, doc.internal.pageSize.width - 70), 21, ry + 12);
    }
    afterRatingY = ry + (shift.rating.feedback ? 22 : 16);
  }

  // Reconciliation banner
  const reconY = afterRatingY + 2;
  let nextY = statusBanner(
    doc, reconY, allOk,
    allOk
      ? 'Shift Fully Balanced'
      : 'Discrepancy Detected' +
        (!walletOk ? '  |  Cash off $' + Math.abs(walletDisc).toFixed(2) : '') +
        (!gameOk   ? '  |  Points off ' + Math.abs(gameDisc) + ' pts' : ''),
    allOk
      ? 'All deposits, cashouts, fees, and expenses reconcile correctly.'
      : 'The numbers do not match. See the reconciliation breakdown below.',
  );
  ...
  if (gameRows.length > 0) {
    ...
    autoTable(doc, {
      ...
      didParseCell(data) {
        if (data.section !== 'body' || data.column.index !== 3) return;
        if (data.cell.raw === 'N/A') { data.cell.styles.textColor = C.muted; return; }
        // Points going down is normal/good (spent on players)
        data.cell.styles.textColor = String(data.cell.raw).startsWith('-') ? C.green : C.red;
        data.cell.styles.fontStyle = 'bold';
      },
    });
    nextY = (doc.lastAutoTable?.finalY ?? nextY + 20) + 5;
  ...
  if (transactions.length > 0) {
    ...
    autoTable(doc, {
      ...
      head: [['Time', 'Player', 'Type', 'Game / Wallet', 'Pts Before -> After', 'Amount', 'Fee', 'Bal. After', 'Status']],
  ...
  if (hasFeedback || effort != null) {
    ...
    const fbFields = [
      ...
      { label: 'Recommendations (Prev)',     value: feedback.recommendationsLastShift },
  ...
  doc.save('shift-report-' + safeDate + '-' + safeName + '.pdf');
}
