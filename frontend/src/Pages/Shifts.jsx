...
const TH = {
    ...
    letterSpacing: '0.4px', borderBottom: '1px solid #e2e8f0', whiteSpace: 'nowrap', background: '#f8fafc',
};
const TD = { padding: '14px 20px', borderBottom: '1px solid #f1f5f9', fontSize: '13px', color: '#0f172a' };
...
export const ShiftsPage = () => {
    ...
    useEffect(() => {
        const fetchShifts = async () => {
            const data = await api.shifts.getShifts(usr?.role);
            setPastShifts(data.data);
        };
        fetchShifts();
    }, []);
    ...
    const handleStartShift = () => {
        shiftStartTimeRef.current = new Date();
        setShiftActive(true);
    };
    ...
    const handleEndShift = async () => {
        ...
        const now = new Date();
        const starttime = shiftStartTimeRef.current;
        const duration = Math.round((now - starttime) / 60000);
        try {
            setLoading(true);
            await api.shifts.createShift({ teamRole: usr?.role || usr, startTime: starttime, endTime: now, duration, isActive: false });
            setShiftActive(false);
            setSuccess('Shift recorded successfully!');
            const data = await api.shifts.getShifts(usr?.role);
            setPastShifts(data.data);
        } catch (err) {
            setError(err.message || 'Failed to record the shift');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: 'inherit' }}>
            ...
            <div style={{
                ...
                <div>
                    ...
                    <p style={{ margin: 0, fontSize: '12px', color: shiftActive ? '#4ade80' : '#b45309', lineHeight: '1.4' }}>
                        {shiftActive ? 'You can now record transactions and bonuses' : 'Start a shift to begin recording transactions and activities'}
                    </p>
                </div>
            </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
                    ...
                    {shiftActive ? (
                        <button onClick={handleEndShift} disabled={loading}
                            style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '12px 24px', background: '#dc2626', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: '700', fontSize: '14px', cursor: loading ? 'wait' : 'pointer', opacity: loading ? 0.7 : 1 }}>
                            {loading ? <><RefreshCw style={{ width: '14px', height: '14px', animation: 'spin 1s linear infinite' }} /> Recording…</> : 'End Shift'}
                        </button>
                    ) : (
                        <button onClick={handleStartShift}
                            style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '12px 24px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: '700', fontSize: '14px', cursor: 'pointer' }}>
                            Start Shift
                        </button>
                    )}
                </div>
            ...
            <div style={{ ...CARD, padding: 0, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '20px 28px', borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <p style={{ margin: 0, fontSize: '12px', color: '#94a3b8' }}>History of completed work shifts ({pastShifts.length})</p>
                </div>
                                ...
                                {pastShifts.map((shift) => (
                                    ...
                                    >
                                        <td style={{ ...TD, fontWeight: '600' }}>
                                            {new Date(shift.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                        </td>
                                        <td style={TD}>
                                            {new Date(shift.startTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}
                                        </td>
                                        <td style={TD}>
                                            {shift.endTime ? new Date(shift.endTime).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }) : '—'}
                                        </td>
                                        <td style={TD}>
                                        ...
                                        <td style={{ ...TD, textAlign: 'center', color: '#94a3b8', fontWeight: '600' }}>—</td>
                                        <td style={{ ...TD, textAlign: 'center' }}>
                                            <button style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '32px', height: '32px', background: '#eff6ff', border: 'none', borderRadius: '8px', cursor: 'pointer', color: '#2563eb', transition: 'background .15s' }}
