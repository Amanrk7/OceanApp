/**
 * MissingPlayerInfoPage.jsx — FIXED
 *
 * Fixes applied:
 *  1. Uses /api/players/missing-info endpoint (not getPlayers)
 *  2. Assign → POST /api/players/:id/assign-missing-info-task (correct backend endpoint)
 *  3. Team members (TEAM1-4) see NO action buttons at all
 *  4. Edit button → navigates to /PlayerDashboard/:id
 *  5. SSE via tasksAPI.connectSSE() with correct base URL
 *  6. Team members fetched from /api/team-members for dropdown
 *  7. Role derived from user prop (not a broken api.auth.me() call)
 */
...
// ─── Contact field definitions (matches backend missingFields keys) ─────────────
const CONTACT_FIELDS = [
  { key: "phone",     label: "Phone",     Icon: Phone,     color: "#0ea5e9", bg: "#f0f9ff", border: "#bae6fd" },
  { key: "email",     label: "Email",     Icon: Mail,      color: "#8b5cf6", bg: "#faf5ff", border: "#ddd6fe" },
  { key: "snapchat",  label: "Snapchat",  Icon: Camera,    color: "#f59e0b", bg: "#fffbeb", border: "#fde68a" },
  { key: "instagram", label: "Instagram", Icon: Instagram, color: "#ec4899", bg: "#fdf2f8", border: "#f9a8d4" },
  { key: "telegram",  label: "Telegram",  Icon: Send,      color: "#06b6d4", bg: "#ecfeff", border: "#a5f3fc" },
];

// Get missing CONTACT fields only (excludes 'assigned_member')
function getMissingContactFields(player) {
  const serverMissing = player.missingFields || [];
  return CONTACT_FIELDS.filter((f) => serverMissing.includes(f.key));
}
...
function Avatar({ name, size = 38 }) {
  const COLORS = [
    ...
    "#0ea5e9","#10b981","#14b8a6","#a855f7","#06b6d4",
  ];
  const idx = name
    ? (name.charCodeAt(0) + (name.charCodeAt(1) || 0)) % COLORS.length
    : 0;
  const initials = name
  ...
  return (
    <div
      style={{
        ...
        flexShrink: 0, fontSize: size * 0.36, fontWeight: "800", color: "#fff",
      }}
    >
      {initials}
    </div>
...
function TierBadge({ tier }) {
  ...
  return (
    <span style={{ padding: "2px 9px", borderRadius: "6px", fontSize: "10px", fontWeight: "800", background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>
      {tier || "—"}
    </span>
  );
}
...
function FieldChip({ field }) {
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: "4px", padding: "3px 8px", borderRadius: "6px", fontSize: "11px", fontWeight: "600", background: bg, border: `1px solid ${border}`, color }}>
      <Icon style={{ width: "10px", height: "10px" }} /> {label}
    </span>
  );
}

// ─── Assign Dropdown (admin only) — uses real team members ────────────────────
function AssignDropdown({ player, teamMembers, onAssign, loading }) {
  ...
  const DOT_COLORS = { TEAM1: "#6366f1", TEAM2: "#10b981", TEAM3: "#f59e0b", TEAM4: "#ef4444" };
    ...
    <div ref={ref} style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          boxShadow: loading ? "none" : "0 2px 10px #8b5cf655",
        }}
      >
        <Users style={{ width: "12px", height: "12px" }} />
        {loading ? "Assigning…" : "Assign To"}
        {!loading && <ChevronDown style={{ width: "11px", height: "11px", transform: open ? "rotate(180deg)" : "none", transition: "transform .2s" }} />}
      </button>

      {open && (
        <div style={{
          position: "absolute", bottom: "calc(100% + 6px)", right: 0, zIndex: 50,
          ...CARD, padding: "5px", minWidth: "180px",
          boxShadow: "0 8px 24px rgba(15,23,42,.14)",
        }}>
          {teamMembers.length === 0 ? (
            <div style={{ padding: "10px 14px", fontSize: "12px", color: "#94a3b8" }}>No team members found</div>
          ) : (
            teamMembers.map((m) => (
              <button
                key={m.id}
                onClick={() => { setOpen(false); onAssign(player, m); }}
                style={{
                  display: "flex", width: "100%", alignItems: "center", gap: "8px",
                  padding: "8px 12px", background: "none", border: "none", cursor: "pointer",
                  borderRadius: "7px", fontSize: "13px", fontWeight: "600", color: "#0f172a",
                  transition: "background .1s", fontFamily: "inherit",
                }}
                onMouseEnter={(e) => { e.currentTarget.style.background = "#f8fafc"; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = "none"; }}
              >
                <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: DOT_COLORS[m.role] || "#94a3b8", flexShrink: 0 }} />
                <span style={{ flex: 1, textAlign: "left" }}>{m.name}</span>
                <span style={{ fontSize: "10px", color: "#94a3b8", fontWeight: "600" }}>{m.role}</span>
              </button>
            ))
          )}
        </div>
...
// ─── Player Card ───────────────────────────────────────────────────────────────
function PlayerCard({ player, isAdmin, teamMembers, onAssign, onEdit, assigning, justDone }) {
  const missingContact = getMissingContactFields(player);
  const isCritical = player.isCritical || missingContact.length >= 2;
  const isUnassigned = !player.assignedToId;
  const assignedName = player.assignedTo?.name || null;
  ...
  const isAssigning = assigning === player.id;
  ...
  return (
    <div style={{
      ...
      border: justDone ? "1px solid #86efac" : isCritical ? "1px solid #fca5a5" : "1px solid #e2e8f0",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
            <span style={{ fontWeight: "800", fontSize: "14px", color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "130px" }}>
              {player.name}
            </span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
        ...
        <span style={{ fontSize: "12px", fontWeight: "600", color: isUnassigned ? "#f87171" : "#10b981" }}>
          {isUnassigned ? "No assigned member" : `Assigned: ${assignedName}`}
        </span>
      </div>
      ...
      {missingContact.length > 0 ? (
        <div>
          <div style={{ fontSize: "10px", fontWeight: "800", color: isCritical ? "#ef4444" : "#94a3b8", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "6px" }}>
            Missing ({missingContact.length})
            {isCritical && (
              <span style={{ marginLeft: "6px", padding: "1px 5px", background: "#fee2e2", borderRadius: "4px", color: "#ef4444" }}>Critical</span>
            )}
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "4px" }}>
            {missingContact.map((f) => <FieldChip key={f.key} field={f} />)}
          </div>
        </div>
      ) : (
          ...
          {isAdmin && (
            justDone ? (
              <span style={{ display: "inline-flex", alignItems: "center", gap: "4px", padding: "5px 11px", background: "#dcfce7", border: "1px solid #86efac", borderRadius: "8px", fontSize: "12px", fontWeight: "700", color: "#166534" }}>
                <CheckCircle style={{ width: "11px", height: "11px" }} /> Assigned!
              </span>
            ) : (
              <AssignDropdown
                player={player}
...
function StatTile({ Icon, label, value, color, bg, border, emphasis }) {
  return (
    <div style={{
      ...CARD, padding: "14px 18px", display: "flex", alignItems: "center", gap: "12px",
      border: emphasis ? `1px solid ${border}` : "1px solid #e2e8f0",
      background: emphasis ? bg : "#fff",
    }}>
...
// ─── Main Page ──────────────────────────────────────────────────────────────────
export default function MissingPlayerInfoPage({ user }) {
  const navigate = useNavigate();

  // Role detection from the user prop (no broken API call needed)
  ...
  // ── Load players with missing info ──────────────────────────────────────────
  const loadPlayers = useCallback(async (silent = false) => {
    try {
      if (!silent) setLoading(true);
      setError("");
      const r = await api.players.getMissingInfo(true);
      setPlayers(r.data || []);
      setApiStats(r.stats || {});
  ...
  const stats = useMemo(() => ({
    ...
    critical:    apiStats.critical    ?? players.filter((p) => p.isCritical).length,
    misSnap:     apiStats.missingSnapchat ?? players.filter((p) => (p.missingFields || []).includes("snapchat")).length,
    misPhone:    apiStats.missingPhone    ?? players.filter((p) => (p.missingFields || []).includes("phone")).length,
    misEmail:    apiStats.missingEmail    ?? players.filter((p) => (p.missingFields || []).includes("email")).length,
    misIg:       players.filter((p) => (p.missingFields || []).includes("instagram")).length,
    misTg:       players.filter((p) => (p.missingFields || []).includes("telegram")).length,
    unassigned:  apiStats.unassigned     ?? players.filter((p) => !p.assignedToId).length,
  }), [players, apiStats]);
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter === "critical")   list = list.filter((p) => p.isCritical);
    else if (activeFilter === "unassigned") list = list.filter((p) => !p.assignedToId);
    else if (activeFilter !== "all") {
      list = list.filter((p) => (p.missingFields || []).includes(activeFilter));
    }
  ...
  // Calls the correct backend endpoint: POST /api/players/:id/assign-missing-info-task
  const handleAssign = async (player, teamMember) => {
    try {
      setAssigning(player.id);
      setError("");

      const priority = (player.isCritical || getMissingContactFields(player).length >= 2) ? "HIGH" : "MEDIUM";

      const res = await fetch(`${API}/players/${player.id}/assign-missing-info-task`, {
        method: "POST",
        credentials: "include",
        headers: getAuthHeaders(true),
        body: JSON.stringify({ assignedToId: teamMember.id, priority }),
      });

      const data = await res.json();
      if (!res.ok) {
        // 409 = task already exists, treat as soft warning
        if (res.status === 409) {
          setSuccess(`ℹ️ ${player.name} already has an active task (assigned to ${teamMember.name}).`);
        } else {
          throw new Error(data.error || "Failed to assign task");
        }
      } else {
        setSuccess(`✓ ${player.name} assigned to ${teamMember.name}. Task added to their queue.`);
      }
      ...
      setPlayers((prev) =>
        prev.map((p) =>
          p.id === player.id
            ? { ...p, assignedToId: teamMember.id, assignedTo: { id: teamMember.id, name: teamMember.name, role: teamMember.role } }
            : p
        )
      );
      ...
      setTimeout(() => {
        ...
        setSuccess("");
        loadPlayers(true); // re-fetch in background to confirm
      }, 4000);

    } catch (e) {
      setError(e?.message || `Failed to assign ${player.name}.`);
    } finally {
      setAssigning(null);
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "12px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: "13px", color: "#94a3b8", marginTop: "2px" }}>
              Players with incomplete contact details
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
          ...
          <div style={{
            ...
            <ShieldAlert style={{ width: "13px", height: "13px", color: isAdmin ? "#92400e" : "#1d4ed8" }} />
            <span style={{ fontSize: "11px", fontWeight: "700", color: isAdmin ? "#92400e" : "#1d4ed8" }}>
          ...
          >
            ...
            <span style={{ fontSize: "11px", fontWeight: "700", color: autoRefresh ? "#166534" : "#64748b" }}>
          ...
          <button
            ...
            style={{
              ...
              border: "1px solid #e2e8f0", borderRadius: "9px", background: "#fff",
              cursor: loading ? "not-allowed" : "pointer", fontWeight: "600", fontSize: "13px", color: "#374151",
              fontFamily: "inherit",
            }}
          >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "10px" }}>
        <StatTile Icon={Users}         label="Total Players"         value={stats.total}      color="#0f172a"  bg="#f8fafc"  border="#e2e8f0" />
      ...
      <div style={{ ...CARD, padding: 0 }}>
        <div style={{ position: "relative" }}>
          ...
          <input
            ...
            style={{ ...INPUT, paddingLeft: "46px", paddingRight: search ? "40px" : "16px", border: "none", borderRadius: "14px", padding: "14px 16px 14px 46px", background: "transparent" }}
          />
          {search && (
            <button
              onClick={() => setSearch("")}
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", alignItems: "center" }}>
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              {tab.dotColor && (
                <span style={{ width: "7px", height: "7px", borderRadius: "50%", background: active ? "#fff" : tab.dotColor, display: "inline-block" }} />
              )}
              {tab.label}
              {tab.count > 0 && (
