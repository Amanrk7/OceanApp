...
// Slot-based colors (cycles for any number of slots)
const SLOT_COLORS = [
  ...
  { bg: '#fafaf9', text: '#44403c', border: '#d6d3d1' },
  { bg: '#fef9c3', text: '#854d0e', border: '#fef08a' },
];
const slotColor = (slot) => SLOT_COLORS[(slot - 1) % SLOT_COLORS.length] ?? SLOT_COLORS[0];
...
const TH = {
  textAlign: 'left', padding: '9px 14px', fontWeight: '600', color: '#64748b',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '1px' }}>
      ...
      <span style={{ fontSize: '10px', fontWeight: '700', color }}>
        {(rating.overallRating ?? 0).toFixed(1)}/5
      </span>
    </div>
  );
}

function MemberSection({ member, allShifts, onRated }) {
  const [expanded, setExpanded] = useState(false);
  const [ratingModal, setRatingModal] = useState(null);

  // ✅ FIX: filter by checkin.userId instead of teamRole
  const shifts = allShifts.filter(s =>
    s.checkin?.userId === member.id ||
    s.performer?.id === member.id
  );

  const rc = slotColor(member.teamSlot ?? 1);
  const rated = shifts.filter(s => s.rating || s.checkin?.rating).length;
  const unrated = shifts.length - rated;
    ...
    <div style={{ border: '1px solid #e2e8f0', borderRadius: '12px', overflow: 'hidden', marginBottom: '10px' }}>
      ...
      <div
        ...
        style={{
          ...
          gap: '12px', cursor: 'pointer', background: '#fafbfc', userSelect: 'none',
        }}
      >
        <div style={{
          ...
          fontSize: '14px', fontWeight: '800', color: rc.text, flexShrink: 0,
        }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '1px', display: 'flex', gap: '6px', alignItems: 'center' }}>
            {/* ✅ FIX: show slot label, not raw role string */}
            <span style={{
              padding: '1px 7px', borderRadius: '5px', fontSize: '10px',
              fontWeight: '700', background: rc.bg, color: rc.text, border: `1px solid ${rc.border}`,
            }}>
              Team {member.teamSlot}
            </span>
            <span>{shifts.length} shifts · {rated} rated</span>
            {unrated > 0 && (
              <span style={{
                padding: '1px 7px', background: '#fffbeb', color: '#b45309',
                borderRadius: '5px', fontSize: '10px', fontWeight: '700', border: '1px solid #fde68a',
              }}>
                {unrated} unrated
              </span>
            )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr>
                  {['Date','Start → End','Duration','Deposits','Cashouts','Net Profit','Effort','Rating','Action'].map(h => (
                    <th key={h} style={{
                      ...TH,
                      textAlign: ['Deposits','Cashouts','Net Profit'].includes(h) ? 'right' : 'left',
                    }}>
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {shifts.map(shift => {
                  ...
                  const rating = shift.rating || shift.checkin?.rating || null;
                    ...
                    >
                      ...
                      <td style={TD}>
                        <span style={{
                          ...
                          background: '#f1f5f9', borderRadius: '6px', fontSize: '12px', color: '#475569',
                        }}>
                      ...
                      <td style={{ ...TD, textAlign: 'center' }}>
                        {effort != null
                          ? <span style={{ fontWeight: '700', color: effortColor }}>{effort}/10</span>
                          : '—'}
                      </td>
                      <td style={TD}><StarRow rating={rating} /></td>
                      <td style={TD}>
                        <button
                          onClick={() => setRatingModal({ shift, memberName: member.name })}
                          style={{
...
export default function AdminTeamShiftsSection() {
  ...
  // ✅ FIX: single fetch for all members + all shifts, then distribute in the child
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [membersRes, shiftsRes] = await Promise.all([
        fj('/team-members'),
        fj('/reports/my-shifts?allMembers=true&limit=100'),
      ]);
      setMembers(
        (membersRes.data ?? []).sort((a, b) => (a.teamSlot ?? 99) - (b.teamSlot ?? 99))
      );
      // Only completed (non-active) shifts
      setAllShifts((shiftsRes.data ?? []).filter(s => !s.isActive));
    } catch (err) {
      console.error('AdminTeamShiftsSection load error:', err);
    } finally {
      setLoading(false);
    }
  }, [refreshKey]);
    ...
    <div style={{
      ...
      <div
        ...
        style={{
          ...
          cursor: 'pointer', borderBottom: expanded ? '1px solid #f1f5f9' : 'none',
          background: '#fafbfc',
        }}
      >
        <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '15px', fontWeight: '700', color: '#0f172a' }}>
            Rate Team Member Shifts
          </div>
          <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '1px' }}>
            Review and rate completed shifts for all team members
          </div>
        </div>
        {expanded
          ? <ChevronUp size={15} color="#94a3b8" />
